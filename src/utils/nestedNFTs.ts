
import { getGalleryNFTs } from './openSeaGalleries';

export const getNestedNFTs = (tokenId: number) => {
  const nestedNFTsMap: Record<number, any[]> = {
    585: [
      {
        id: 'gone-green-585',
        name: 'Gone Green #585',
        image: '/lovable-uploads/d797b860-3cc3-4029-9f00-de606955ab3e.png',
        collection: 'Gone Green',
        zoraUrl: 'https://zora.co/@chonk585',
        contractAddress: '0x8bc6e5e303344f5526057df842316ff4c347efd7'
      },
      {
        id: 'basepaint-day-15',
        name: 'BasePaint Day #15',
        image: '/lovable-uploads/511c077d-c71d-478b-a63f-af40c4f941cb.png',
        collection: 'BasePaint',
        openSeaUrl: 'https://opensea.io/item/base/0xba5e05cb26b78eda3a2f8e3b3814726305dcac83/15'
      },
      {
        id: 'klima-stake-585',
        name: '100 $KLIMA Staked',
        image: '/lovable-uploads/b3b222f0-1d06-420e-907c-0b88ca3114e2.png',
        collection: 'Klima Protocol'
      }
    ],
    586: [
      {
        id: 'gone-green-586',
        name: 'Gone Green #586',
        image: '/lovable-uploads/c38e2fdb-061a-4594-9810-26381a574c8e.png',
        collection: 'Gone Green',
        zoraUrl: 'https://zora.co/@chonk586',
        contractAddress: '0xcbe47fa36e99d11125660262611a1fc998f330b5'
      },
      {
        id: 'basepaint-day-670',
        name: 'BasePaint Day #670',
        image: '/lovable-uploads/1fdf376b-997a-45c0-80c4-2f30b4047c76.png',
        collection: 'BasePaint',
        openSeaUrl: 'https://opensea.io/item/base/0xba5e05cb26b78eda3a2f8e3b3814726305dcac83/670'
      },
      {
        id: 'klima-stake-586',
        name: '100 $KLIMA Staked',
        image: '/lovable-uploads/d1793031-6834-4186-81dc-108975cd1022.png',
        collection: 'Klima Protocol'
      }
    ],
    588: [
      {
        id: 'gone-green-588',
        name: 'Gone Green #588',
        image: '/lovable-uploads/bf594925-cfe3-4c85-aea2-99e87e52bc3f.png',
        collection: 'Gone Green',
        zoraUrl: 'https://zora.co/@chonk588',
        contractAddress: '0x021594a8005aec083f04b53edf2e57e941086d5e'
      },
      {
        id: 'basepaint-day-276',
        name: 'BasePaint Day #276',
        image: '/lovable-uploads/bec1d52f-4cd9-4558-95eb-65c24cc45f76.png',
        collection: 'BasePaint',
        openSeaUrl: 'https://opensea.io/item/base/0xba5e05cb26b78eda3a2f8e3b3814726305dcac83/276'
      },
      {
        id: 'klima-stake-588',
        name: '100 $KLIMA Staked',
        image: '/lovable-uploads/4a4b3cd6-9bde-4e5d-80d1-302d1d96ed99.png',
        collection: 'Klima Protocol'
      }
    ],
    596: [
      {
        id: 'gone-green-596',
        name: 'Gone Green #596',
        image: '/lovable-uploads/50e134f7-a136-4942-9b36-e22388de4367.png',
        collection: 'Gone Green',
        zoraUrl: 'https://zora.co/@chonk596',
        contractAddress: '0x09d5b3297545f69a8893bb7a610132354117b66e'
      },
      {
        id: 'klima-stake-596',
        name: '99 $KLIMA Staked',
        image: '/lovable-uploads/ba785906-0857-44c3-9286-28d7955cd5c7.png',
        collection: 'Klima Protocol'
      },
      {
        id: 'basepaint-day-485',
        name: 'BasePaint Day #485',
        image: '/lovable-uploads/f45142a4-0651-4e29-8814-9dca1a418963.png',
        collection: 'BasePaint',
        openSeaUrl: 'https://opensea.io/item/base/0xba5e05cb26b78eda3a2f8e3b3814726305dcac83/485'
      },
      {
        id: 'nine-chonks-1',
        name: 'NINE CHONKS 🎁 #1',
        image: '/lovable-uploads/4168b385-def6-4539-a53b-4199c24a41cd.png',
        collection: 'NINE CHONKS',
        openSeaUrl: 'https://opensea.io/item/base/0x4533907a0b8b8528d4d0290f15f2f90a5dec7bed/1'
      },
      {
        id: 'nine-chonks-2',
        name: 'NINE CHONKS 🎁 #2',
        image: '/lovable-uploads/c2e3f237-b22b-4679-a8fb-f78f714d0125.png',
        collection: 'NINE CHONKS',
        openSeaUrl: 'https://opensea.io/item/base/0x4533907a0b8b8528d4d0290f15f2f90a5dec7bed/2'
      },
      {
        id: 'nine-chonks-3',
        name: 'NINE CHONKS 🎁 #3',
        image: '/lovable-uploads/79a68dd8-bd3a-4565-80d5-12dac0713bfb.png',
        collection: 'NINE CHONKS',
        openSeaUrl: 'https://opensea.io/item/base/0x4533907a0b8b8528d4d0290f15f2f90a5dec7bed/3'
      },
      {
        id: 'nine-chonks-4',
        name: 'NINE CHONKS 🎁 #4',
        image: '/lovable-uploads/42d78e09-011b-4ead-a4fa-bfdff93f482d.png',
        collection: 'NINE CHONKS',
        openSeaUrl: 'https://opensea.io/item/base/0x4533907a0b8b8528d4d0290f15f2f90a5dec7bed/4'
      }
    ],
    599: [
      {
        id: 'gone-green-599',
        name: 'Gone Green #599',
        image: '/lovable-uploads/82ec3bc0-b45c-4690-ae19-f9c61b898daf.png',
        collection: 'Gone Green',
        zoraUrl: 'https://zora.co/@chonk599',
        contractAddress: '0xe995b8f87c76614fd094acc971d1651ab82f6a2a'
      },
      {
        id: 'basepaint-day-209',
        name: 'BasePaint Day #209',
        image: '/lovable-uploads/50c39ba2-c2b0-49e4-ba60-ef82c9e60ba5.png',
        collection: 'BasePaint',
        openSeaUrl: 'https://opensea.io/item/base/0xba5e05cb26b78eda3a2f8e3b3814726305dcac83/209'
      },
      {
        id: 'klima-stake-599',
        name: '100 $KLIMA Staked',
        image: '/lovable-uploads/deac98a4-071e-46a4-9913-a9b094936bdc.png',
        collection: 'Klima Protocol'
      }
    ],
    601: [
      {
        id: 'gone-green-601',
        name: 'Gone Green #601',
        image: '/lovable-uploads/1c55caa2-5b4a-41a1-ba5b-00a23b5aba74.png',
        collection: 'Gone Green',
        zoraUrl: 'https://zora.co/@chonk601',
        contractAddress: '0xd0c95dca0101eca9725aed891bda0a2b1a394e38'
      },
      {
        id: 'basepaint-day-209-601',
        name: 'BasePaint Day #209',
        image: '/lovable-uploads/b65137dd-96f1-4b7b-ab92-1deff95d5bd1.png',
        collection: 'BasePaint',
        openSeaUrl: 'https://opensea.io/item/base/0xba5e05cb26b78eda3a2f8e3b3814726305dcac83/209'
      },
      {
        id: 'klima-stake-601',
        name: '100 $KLIMA Staked',
        image: '/lovable-uploads/9546ef85-5991-4c07-9718-b259b3c74e33.png',
        collection: 'Klima Protocol'
      }
    ],
    606: [
      {
        id: 'gone-green-606',
        name: 'Gone Green #606',
        image: '/lovable-uploads/ef9c1e7c-9349-4eb2-9c43-ef8f43449a55.png',
        collection: 'Gone Green',
        zoraUrl: 'https://zora.co/@chonk606',
        contractAddress: '0xdf042a1398377f9ae2d3b482bb2e1aba9bb8da01'
      },
      {
        id: 'basepaint-day-669',
        name: 'BasePaint Day #669',
        image: '/lovable-uploads/3ea8d7f2-2429-439d-9645-f70cd2798240.png',
        collection: 'BasePaint',
        openSeaUrl: 'https://opensea.io/item/base/0xba5e05cb26b78eda3a2f8e3b3814726305dcac83/669'
      },
      {
        id: 'klima-stake-606',
        name: '100 $KLIMA Staked',
        image: '/lovable-uploads/9546ef85-5991-4c07-9718-b259b3c74e33.png',
        collection: 'Klima Protocol'
      }
    ],
    662: [
      {
        id: 'gone-green-662',
        name: 'Gone Green #662',
        image: '/lovable-uploads/9dca13bc-eed2-4aad-a372-7648b6c7e95c.png',
        collection: 'Gone Green',
        zoraUrl: 'https://zora.co/@chonk662',
        contractAddress: '0xbef0550be11c727cdf0ee6a9b4c6616b0aaff334'
      },
      {
        id: 'basepaint-day-233',
        name: 'BasePaint Day #233',
        image: '/lovable-uploads/deb8ca1a-d5ad-4787-9509-b66c6c9c859a.png',
        collection: 'BasePaint',
        openSeaUrl: 'https://opensea.io/item/base/0xba5e05cb26b78eda3a2f8e3b3814726305dcac83/233'
      },
      {
        id: 'klima-stake-662',
        name: '100 $KLIMA Staked',
        image: '/lovable-uploads/9546ef85-5991-4c07-9718-b259b3c74e33.png',
        collection: 'Klima Protocol'
      }
    ],
    663: [
      {
        id: 'gone-green-663',
        name: 'Gone Green #663',
        image: '/lovable-uploads/203aff55-05a6-4206-8547-21930e7f11bd.png',
        collection: 'Gone Green',
        zoraUrl: 'https://zora.co/@chonk663',
        contractAddress: '0xf44591c8044471dd293e8a538189b6596f2795ef'
      },
      {
        id: 'basepaint-day-197',
        name: 'BasePaint Day #197',
        image: '/lovable-uploads/0d998e54-3352-49ca-882c-3892c4d70602.png',
        collection: 'BasePaint',
        openSeaUrl: 'https://opensea.io/item/base/0xba5e05cb26b78eda3a2f8e3b3814726305dcac83/197'
      },
      {
        id: 'klima-stake-663',
        name: '100 $KLIMA Staked',
        image: '/lovable-uploads/9546ef85-5991-4c07-9718-b259b3c74e33.png',
        collection: 'Klima Protocol'
      }
    ],
    665: [
      {
        id: 'gone-green-665',
        name: 'Gone Green #665',
        image: '/lovable-uploads/f03fadfc-4f72-4926-8fa0-2843426fb6da.png',
        collection: 'Gone Green',
        zoraUrl: 'https://zora.co/@chonk665',
        contractAddress: '0x44a86e54e3e49e2fca3c64e4f1fc78199349324c'
      },
      {
        id: 'basepaint-day-215',
        name: 'BasePaint Day #215',
        image: '/lovable-uploads/d039653a-f24c-48bf-8699-8072340eab4c.png',
        collection: 'BasePaint',
        openSeaUrl: 'https://opensea.io/item/base/0xba5e05cb26b78eda3a2f8e3b3814726305dcac83/215'
      },
      {
        id: 'klima-stake-665',
        name: '100 $KLIMA Staked',
        image: '/lovable-uploads/9546ef85-5991-4c07-9718-b259b3c74e33.png',
        collection: 'Klima Protocol'
      }
    ],
    672: [
      {
        id: 'gone-green-672',
        name: 'Gone Green #672',
        image: '/lovable-uploads/337b7708-ebcf-4696-ac2e-07e46315a15e.png',
        collection: 'Gone Green',
        zoraUrl: 'https://zora.co/@chonk672',
        contractAddress: '0x84fbbecd652c9f8e35cf2be29d5a5c93d1582e89'
      },
      {
        id: 'basepaint-day-209-672',
        name: 'BasePaint Day #209',
        image: '/lovable-uploads/5247086c-a123-42dc-b53c-b082a1035cf6.png',
        collection: 'BasePaint',
        openSeaUrl: 'https://opensea.io/item/base/0xba5e05cb26b78eda3a2f8e3b3814726305dcac83/209'
      },
      {
        id: 'klima-stake-672',
        name: '100 $KLIMA Staked',
        image: '/lovable-uploads/9546ef85-5991-4c07-9718-b259b3c74e33.png',
        collection: 'Klima Protocol'
      }
    ],
    676: [
      {
        id: 'gone-green-676',
        name: 'Gone Green #676',
        image: '/lovable-uploads/ceb85d63-f5aa-436d-93af-7f5e2db6969c.png',
        collection: 'Gone Green',
        zoraUrl: 'https://zora.co/@chonk676',
        contractAddress: '0x89735823f88b141a5f50646ccd1517c92b6d45ce'
      },
      {
        id: 'basepaint-day-246',
        name: 'BasePaint Day #246',
        image: '/lovable-uploads/5c226b2e-7e22-415e-9e9d-ddcc7cf86f5f.png',
        collection: 'BasePaint',
        openSeaUrl: 'https://opensea.io/item/base/0xba5e05cb26b78eda3a2f8e3b3814726305dcac83/246'
      },
      {
        id: 'klima-stake-676',
        name: '100 $KLIMA Staked',
        image: '/lovable-uploads/9546ef85-5991-4c07-9718-b259b3c74e33.png',
        collection: 'Klima Protocol'
      }
    ],
    678: [
      {
        id: 'gone-green-678',
        name: 'Gone Green #678',
        image: '/lovable-uploads/c7d6c82d-b7a4-4308-8606-2cde3aa9f608.png',
        collection: 'Gone Green',
        zoraUrl: 'https://zora.co/@chonk678',
        contractAddress: '0xd9dd283d2e3646e6c2795a24932a14adbd1cd788'
      },
      {
        id: 'basepaint-day-241',
        name: 'BasePaint Day #241',
        image: '/lovable-uploads/bd6919f9-872f-4477-b176-535bac5a7364.png',
        collection: 'BasePaint',
        openSeaUrl: 'https://opensea.io/item/base/0xba5e05cb26b78eda3a2f8e3b3814726305dcac83/241'
      },
      {
        id: 'klima-stake-678',
        name: '100 $KLIMA Staked',
        image: '/lovable-uploads/9546ef85-5991-4c07-9718-b259b3c74e33.png',
        collection: 'Klima Protocol'
      }
    ],
    680: [
      {
        id: 'gone-green-680',
        name: 'Gone Green #680',
        image: '/lovable-uploads/7a46b1a8-fb49-4e33-89c5-0e6b72b5e94c.png',
        collection: 'Gone Green',
        zoraUrl: 'https://zora.co/@chonk680',
        contractAddress: '0x25046b4f5a5a15682a01d5324c3c77340d5b8ad1'
      },
      {
        id: 'basepaint-day-225',
        name: 'BasePaint Day #225',
        image: '/lovable-uploads/d8678613-ce25-4ebf-aec0-51f9c77bf1b6.png',
        collection: 'BasePaint',
        openSeaUrl: 'https://opensea.io/item/base/0xba5e05cb26b78eda3a2f8e3b3814726305dcac83/225'
      },
      {
        id: 'klima-stake-680',
        name: '100 $KLIMA Staked',
        image: '/lovable-uploads/9546ef85-5991-4c07-9718-b259b3c74e33.png',
        collection: 'Klima Protocol'
      }
    ],
    681: [
      {
        id: 'gone-green-681',
        name: 'Gone Green #681',
        image: '/lovable-uploads/23d17a8a-8db3-4535-a04d-3842da1b8407.png',
        collection: 'Gone Green',
        zoraUrl: 'https://zora.co/@chonk681',
        contractAddress: '0xcd6f89037377a917615f13f166677b406ded550b'
      },
      {
        id: 'basepaint-day-230',
        name: 'BasePaint Day #230',
        image: '/lovable-uploads/7a162340-a7c4-4802-a099-6a3b6a62c424.png',
        collection: 'BasePaint',
        openSeaUrl: 'https://opensea.io/item/base/0xba5e05cb26b78eda3a2f8e3b3814726305dcac83/230'
      },
      {
        id: 'klima-stake-681',
        name: '100 $KLIMA Staked',
        image: '/lovable-uploads/9546ef85-5991-4c07-9718-b259b3c74e33.png',
        collection: 'Klima Protocol'
      }
    ],
    693: [
      {
        id: 'gone-green-693',
        name: 'Gone Green #693',
        image: '/lovable-uploads/6ae587b6-de3a-43e1-ac42-5b76f0076c44.png',
        collection: 'Gone Green',
        zoraUrl: 'https://zora.co/@chonk693',
        contractAddress: '0x7fd8b61eada4eb171cf08c3ed12e29bf32727c8d'
      },
      {
        id: 'basepaint-day-47',
        name: 'BasePaint Day #47',
        image: '/lovable-uploads/c5e131a0-ce1d-43b7-b8e7-f97f6284c485.png',
        collection: 'BasePaint',
        openSeaUrl: 'https://opensea.io/item/base/0xba5e05cb26b78eda3a2f8e3b3814726305dcac83/47'
      },
      {
        id: 'klima-stake-693',
        name: '100 $KLIMA Staked',
        image: '/lovable-uploads/9546ef85-5991-4c07-9718-b259b3c74e33.png',
        collection: 'Klima Protocol'
      }
    ],
    700: [
      {
        id: 'gone-green-700',
        name: 'Gone Green #700',
        image: '/lovable-uploads/3c37a26e-18da-4f5d-971d-bfb366969143.png',
        collection: 'Gone Green',
        zoraUrl: 'https://zora.co/@chonk700',
        contractAddress: '0xd975a37a3710f2edf70f514af9709e62d9b61ee5'
      },
      {
        id: 'basepaint-day-239',
        name: 'BasePaint Day #239',
        image: '/lovable-uploads/bac65af0-f626-4982-a786-cb46a1416b55.png',
        collection: 'BasePaint',
        openSeaUrl: 'https://opensea.io/item/base/0xba5e05cb26b78eda3a2f8e3b3814726305dcac83/239'
      },
      {
        id: 'klima-stake-700',
        name: '100 $KLIMA Staked',
        image: '/lovable-uploads/9546ef85-5991-4c07-9718-b259b3c74e33.png',
        collection: 'Klima Protocol'
      }
    ],
    697: [
      {
        id: 'gone-green-697',
        name: 'Gone Green #697',
        image: '/lovable-uploads/aba91b1a-f66a-43ec-a6b1-8edc48137296.png',
        collection: 'Gone Green',
        zoraUrl: 'https://zora.co/@chonk697',
        contractAddress: '0x12ea7232bb05e031a0ac588662fac0b2d2a93dbe'
      },
      {
        id: 'basepaint-day-234',
        name: 'BasePaint Day #234',
        image: '/lovable-uploads/441fe014-d01f-4f56-90b4-bb389238f5ee.png',
        collection: 'BasePaint',
        openSeaUrl: 'https://opensea.io/item/base/0xba5e05cb26b78eda3a2f8e3b3814726305dcac83/234'
      },
      {
        id: 'klima-stake-697',
        name: '100 $KLIMA Staked',
        image: '/lovable-uploads/9546ef85-5991-4c07-9718-b259b3c74e33.png',
        collection: 'Klima Protocol'
      }
    ],
    972: [
      {
        id: 'gone-green-972',
        name: 'Gone Green #972',
        image: '/lovable-uploads/e45c4634-41b3-4743-a429-c03e46aef48e.png',
        collection: 'Gone Green',
        zoraUrl: 'https://zora.co/coin/base:0x8056d4b41db338c6c4e792527f80e73ef54cb094',
        contractAddress: '0x8056d4b41db338c6c4e792527f80e73ef54cb094'
      },
      {
        id: 'basepaint-day-247',
        name: 'BasePaint Day #247',
        image: '/lovable-uploads/79a68462-25e2-42b9-bc70-fc6142c11935.png',
        collection: 'BasePaint',
        openSeaUrl: 'https://opensea.io/item/base/0xba5e05cb26b78eda3a2f8e3b3814726305dcac83/247'
      },
      {
        id: 'klima-stake-972',
        name: '100 $KLIMA Staked',
        image: '/lovable-uploads/9546ef85-5991-4c07-9718-b259b3c74e33.png',
        collection: 'Klima Protocol'
      }
    ],
    9534: [
      {
        id: 'gone-green-9534',
        name: 'Gone Green #9534',
        image: '/lovable-uploads/868f00bb-0566-4337-9b24-0c1c97f2fbd7.png',
        collection: 'Gone Green',
        zoraUrl: 'https://zora.co/@chonk9534',
        contractAddress: '0xc32913cebf6d266a86e4b613927743171ccd174b'
      },
      {
        id: 'klima-stake-9534',
        name: '100 $KLIMA Staked',
        image: '/lovable-uploads/9546ef85-5991-4c07-9718-b259b3c74e33.png',
        collection: 'Klima Protocol'
      },
      {
        id: 'basepaint-day-201',
        name: 'BasePaint Day #201',
        image: '/lovable-uploads/ade97e97-5961-45e3-85c7-da242351c633.png',
        collection: 'Base Paint',
        openSeaUrl: 'https://opensea.io/item/base/0xba5e05cb26b78eda3a2f8e3b3814726305dcac83/201'
      }
    ]
  };

  // Get the base nested NFTs for this token
  const baseNFTs = nestedNFTsMap[tokenId] || [];
  
  // Get gallery NFTs for this token
  const galleryNFTs = getGalleryNFTs(tokenId);
  
  // Combine base NFTs with gallery NFTs
  return [...baseNFTs, ...galleryNFTs];
};
