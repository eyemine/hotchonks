  ...
  return (
    <Card className="group bg-card border-border hover:border-bio-green/40 transition-all duration-500 hover:shadow-bio">
      ...
      <div className="p-4 border-t border-border">
        ...
        >
          <a 
            href={nft.name?.includes('#599') ? 'https://www.purebpm.com/@forked' : nft.name?.includes('#697') ? 'https://www.purebpm.com/@red-hammer' : nft.name?.includes('#678') ? 'https://www.purebpm.com/@sandy-freeland' : nft.name?.includes('#588') ? 'https://www.purebpm.com/@pearce-resurgance' : 'https://purebpm.com'} 
            target="_blank" 
            rel="noopener noreferrer"
          >
            <ExternalLink size={16} className="mr-2" />
            <span className={nft.name?.includes('#599') || nft.name?.includes('#697') || nft.name?.includes('#678') || nft.name?.includes('#588') ? 'font-bold' : ''}>
              {nft.name?.includes('#599') ? "PureBPM Virtual Artist – 'FORKED'" : nft.name?.includes('#697') ? "PureBPM Virtual Artist – 'Red Hammer'" : nft.name?.includes('#678') ? "PureBPM Virtual Artist – 'Sandy Freeland'" : nft.name?.includes('#588') ? "PureBPM Virtual Artist – 'Pearce Resurgance'" : 'PureBPM.com'}
            </span>
          </a>
        </Button>
      </div>
